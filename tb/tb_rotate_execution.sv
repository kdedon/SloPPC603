...
module tb_rotate_execution;
  ...
  task automatic run_case(
    ...
    dispatch_valid = 1'b1;
    #1;
    require(dispatch_ready, "rotate dispatch unexpectedly blocked");
    @(posedge clk);
    #1;
    dispatch_valid = 1'b0;
    ...
    repeat (2) begin
      ...
      require(!issue_valid && !result_valid,
              "pending rotate source issued before wake");
    end
    ...
    require(issue_valid && issue.op == operation &&
            ...
            "RS lost rotate operands, controls, SO, or producer");
    @(posedge clk);
    #1;
    wake_valid = 1'b0;
    ...
    require(result_valid && result == expected_packet,
            "rotate complete result packet mismatch");
    stalled_packet = result;
    repeat (3) begin
      @(posedge clk);
      #1;
      require(result_valid && result == stalled_packet,
              "stalled rotate result packet changed");
    end
    ...
    #1;
    result_ready = 1'b0;
    require(!result_valid, "rotate result was delivered twice");
  endtask
  ...
  initial begin
    ...
    @(negedge clk);
    rst_n = 1'b1;
    require(IQ_DEPTH == 6, "rotate execution fixture resource assumption");

    // Literal answers exercise mask after rotation, ignored high count bits,
    // both captured SO values, and Rc=0 side-effect suppression.
    run_case(ALU_ROTATE, 32'h8000_0001, 32'd0, 32'hffff_ffff,
             1'b0, 1'b0, 1'b0, 1'b0, 1'b1,
             32'h8000_0001, 1'b0, 1'b0, 1'b0, 4'h8);
    run_case(ALU_ROTATE, 32'h8000_0001, 32'd31, 32'hffff_ffff,
             1'b0, 1'b1, 1'b0, 1'b0, 1'b1,
             32'hc000_0000, 1'b0, 1'b0, 1'b0, 4'h9);
    run_case(ALU_ROTATE, 32'h1234_5678, 32'd0, 32'hff00_00ff,
             1'b0, 1'b1, 1'b0, 1'b0, 1'b1,
             32'h1200_0078, 1'b0, 1'b0, 1'b0, 4'h5);
    run_case(ALU_ROTATE, 32'h8000_0000, 32'd0, 32'h7fff_ffff,
             1'b0, 1'b1, 1'b0, 1'b0, 1'b1,
             32'h0000_0000, 1'b0, 1'b0, 1'b0, 4'h3);
    run_case(ALU_ROTATE, 32'h8000_0000, 32'hffff_ffe1, 32'hffff_ffff,
             1'b1, 1'b0, 1'b0, 1'b0, 1'b1,
             32'h0000_0001, 1'b0, 1'b0, 1'b0, 4'h4);
    run_case(ALU_ROTATE, 32'h8000_0001, 32'd32, 32'h8000_0001,
             1'b1, 1'b1, 1'b0, 1'b0, 1'b0,
             32'h8000_0001, 1'b0, 1'b0, 1'b0, 4'h0);
    run_case(ALU_ROTATE, 32'h8000_0001, 32'd63, 32'h8000_0001,
             1'b0, 1'b0, 1'b0, 1'b0, 1'b1,

// Compiled CPU TLB loads then TLBIE: both DTLB ways and indexed ITLB removed; neighboring set survives.
// Physical byte RAM uses independent delayed/backpressured instruction/data ports.
/* verilator lint_off BLKSEQ */
module tb_compiled_tlbload_firmware;
  import ppc_pkg::*;
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      if(tv&&tr)begin
        check(!retired.alignment_exception&&retired.fetch_fault==FETCH_OK&&retired.data_fault==DATA_OK,"retirement diagnostic class");
        if(retired.illegal)begin
          check(mode!=1&&mailbox_retired&&tlbie_retires==2&&page_miss_o&&
                retired.pc==BASE+32'h7000&&retired.insn==32'h80630000&&
                !retired.gpr_write&&!retired.update_write,"unexpected diagnostic retirement");
      ...
      if(halted&&mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        ...
        check(tlbie_retires==2&&terminal_diagnostics==((mode==1)?0:1),"missing terminal invalidation diagnostic");
        $display("PASS compiled CPU TLB-load firmware: mode=%0d TLBIE=%0d diagnostics=%0d Ipage_retires=%0d Ipage_fetches=%0d SRwrites=%0d SRreads=%0d BATwrites=%0d EXT=%0d DEC=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",mode,tlbie_retires,terminal_diagnostics,page_retires,page_fetches,segment_writes,segment_reads,bat_writes,interrupts,decrements,retires,reads,writes,cycles);$finish;
      end
    end
  end

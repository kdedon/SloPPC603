...
module tb_completion_cr_bits;
  ...
  initial begin
    ...
    // Bit writes cannot smuggle a full value or unallocated field mask.
    for (int bit_number=0; bit_number<32; bit_number++) begin
      for (int value=0; value<2; value++) begin
        packet=make_allocation(bit_number,1'b0,1'b1,1'b0,1'b0,1'b0);
        packet.gpr_write=1'b0; packet.write_cr_bit=1'b1; packet.cr_bit=5'(bit_number);
        packet.cr_mask=8'hff; // No multi-field permission: this must be cleared.
        held_cr=cr; held_xer=xer;
        candidate=32'hfffffffe | 32'(value);
        expected_delta=32'(value) << (31-bit_number);
        expected_cr=(cr & ~(32'h80000000 >> bit_number)) | expected_delta;
        allocate_packet(packet,owner_tag);
        send_result(owner_tag,candidate,1'b1,1'b1,1'b1,4'hf,1'b1,1'b0);
        require(retire_valid && retired.write_cr_bit && retired.cr_bit==5'(bit_number) &&
                !retired.write_cr_fields && retired.cr_mask==0 &&
                retired.cr_delta==expected_delta && retired.xer_delta==0 && !retired.gpr_write,
                "bit write lost allocation mask or leaked result bits");
        require(cr==held_cr && xer==held_xer,"bit write changed flags before retirement");
        repeat(2)begin @(posedge clk); #1; require(cr==held_cr && xer==held_xer,"stalled bit write changed flags");end
        accept_retirement();
        require(cr==expected_cr && xer==held_xer && !flags_busy,"bit commit changed unselected state");
      end
    end
    packet=make_allocation(0,1'b1,1'b1,1'b1,1'b1,1'b1);
    packet.write_cr_bit=1'b1; packet.cr_bit=5'd31;
    allocate_packet(packet,illegal_tag);
    require(retired.illegal && !retired.write_cr_bit && retired.cr_bit==0 &&
            retired.cr_delta==0 && !retired.needs_flags,"illegal allocation retained bit permission");
    accept_retirement();
    ...
    $display("PASS completion CR bits: sanitization, identity, masks, recovery, fill/wrap (%0d checks)", checks);
    $finish;
  end

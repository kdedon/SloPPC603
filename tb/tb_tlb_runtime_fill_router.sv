...
module tb_tlb_runtime_fill_router #(parameter bit ENABLE_FILL = 1'b1);
  ...
  task automatic fill_offer(input bit bank, input logic [31:0] ea,
      input logic [23:0] vsid, input bit way, input logic [19:0] rpn,
      input bit changed, input logic [3:0] wimg, input logic [1:0] pp,
      input bit expect_error);
    @(negedge clk_i);
    tlb_fill_req_valid_i = 1; tlb_fill_req_bank_i = bank;
    tlb_fill_req_ea_i = ea; tlb_fill_req_vsid_i = vsid;
    tlb_fill_req_way_i = way; tlb_fill_req_rpn_i = rpn;
    tlb_fill_req_c_i = changed; tlb_fill_req_wimg_i = wimg;
    ...
    check(tlb_fill_rsp_valid_o &&
          ...
          "fill response/error/owner mismatch");
    @(negedge clk_i); tlb_fill_req_valid_i = 0;
  endtask

  task automatic fill_consume;
    @(negedge clk_i); tlb_fill_rsp_ready_i = 1;
    @(posedge clk_i); #1;
    check(!tlb_fill_rsp_valid_o && !tlb_fill_idle_o,
          "fill response did not drain under owner");
    @(negedge clk_i); tlb_fill_rsp_ready_i = 0;
  endtask

  task automatic fill_commit;
    @(negedge clk_i); tlb_fill_commit_i = 1;
    @(posedge clk_i); #1;
    check(tlb_fill_ack_valid_o && !tlb_fill_idle_o,
          "fill commit did not produce held ack");
    @(negedge clk_i); tlb_fill_commit_i = 0;
  endtask

  task automatic fill_ack_and_idle;
    @(negedge clk_i); tlb_fill_ack_ready_i = 1;
    @(posedge clk_i); #1;
    check(!tlb_fill_ack_valid_o && !tlb_fill_idle_o,
          "fill owner released on ack edge");
    @(negedge clk_i); tlb_fill_ack_ready_i = 0;
    @(posedge clk_i); #1;
    check(tlb_fill_idle_o, "fill owner did not release");
  endtask

  task automatic fill_abort_and_idle;
    @(negedge clk_i); tlb_fill_abort_i = 1;
    @(posedge clk_i); #1;
    check(!tlb_fill_ack_valid_o, "aborted fill produced ack");
    @(negedge clk_i); tlb_fill_abort_i = 0;
    if (tlb_fill_rsp_valid_o) fill_consume();
    @(posedge clk_i); #1;
    check(tlb_fill_idle_o, "aborted fill did not release owner");
  endtask

  task automatic fill_entry(input bit bank, input logic [31:0] ea,
      input logic [23:0] vsid, input bit way, input logic [19:0] rpn,
      input bit changed, input logic [3:0] wimg, input logic [1:0] pp);
  ...
  initial begin
    ...
    if (!ENABLE_FILL) begin
      @(negedge clk_i);
      tlb_fill_req_valid_i = 1; tlb_fill_req_bank_i = 1;
      tlb_fill_req_ea_i = EA; tlb_fill_req_vsid_i = VSID_A;
      #1;
      check(!tlb_fill_req_ready_o && !tlb_fill_rsp_valid_o &&
            !tlb_fill_ack_valid_o && tlb_fill_idle_o,
            "disabled fill pins affected router");
      $display("PASS disabled runtime TLB fill router: %0d checks", checks);
      $finish;
    end else begin
      // An already offered real-mode memory request outranks a new fill.
      @(negedge clk_i);
      dmem_req_valid_i = 1; dmem_req_addr_i = 32'h0000_1234;
      tlb_fill_req_valid_i = 1; tlb_fill_req_ea_i = EA;
      #1; check(dmem_req_ready_o && !tlb_fill_req_ready_o,
                "waiting fill displaced old memory offer");
      @(posedge clk_i);
      @(negedge clk_i); dmem_req_valid_i = 0;
      wait (pdmem_req_valid_o);
      #1; check(pdmem_req_addr_o == 32'h0000_1234 &&
                !tlb_fill_req_ready_o, "old memory offer changed");
      pdmem_req_ready_i = 1;
      @(posedge clk_i);
      ...
      @(posedge clk_i);
      @(negedge clk_i); pdmem_rsp_valid_i = 0;
      tlb_fill_req_valid_i = 0;
      #1; check(tlb_fill_idle_o, "fill owner leaked before handshake");

      // Cancellation on the accepted offer edge suppresses the service
      // reservation while still returning the accepted response for drain.
      @(negedge clk_i);
      tlb_fill_req_valid_i = 1; tlb_fill_req_bank_i = 1;
      tlb_fill_req_ea_i = 32'h1000_9234; tlb_fill_req_vsid_i = VSID_A;
      tlb_fill_req_way_i = 0; tlb_fill_req_rpn_i = 20'h91234;
      tlb_fill_req_c_i = 1; tlb_fill_req_wimg_i = 4'h3;
      tlb_fill_req_pp_i = 2'b10; tlb_fill_abort_i = 1;
      context_valid_i = 1;
      ...
      @(negedge clk_i);
      tlb_fill_req_valid_i = 0; tlb_fill_abort_i = 0;
      context_valid_i = 0;
      fill_consume();
      @(posedge clk_i); #1;
      check(tlb_fill_idle_o, "same-edge canceled offer did not drain");

      // The existing CPU tlbie path also suppresses a proposal when abort
      // coincides with the accepted offer edge.
      @(negedge clk_i);
      tlb_inv_req_valid_i = 1; tlb_inv_req_ea_i = 32'h1001_f234;
      tlb_inv_abort_i = 1;
      ...
      @(negedge clk_i);
      tlb_inv_req_valid_i = 0; tlb_inv_abort_i = 0;
      tlb_inv_rsp_ready_i = 1;
      @(posedge clk_i); #1;
      check(!tlb_inv_rsp_valid_o && !tlb_inv_ack_valid_o,
            "canceled TLBIE response did not drain");
      @(negedge clk_i); tlb_inv_rsp_ready_i = 0;
      @(posedge clk_i); #1;
      check(tlb_inv_idle_o, "same-edge canceled TLBIE owner leaked");

      // Exact bank/way/set writes and a lookup through the real page path.
      fill_entry(1, EA, VSID_A, 0, RPN_A, 1, 4'h6, 2'b10);
      fill_entry(1, EA, VSID_B, 1, RPN_B, 1, 4'h2, 2'b10);
      fill_entry(0, EA, VSID_A, 0, RPN_B, 1, 4'h4, 2'b10);
      fill_entry(1, 32'h1000_2234, VSID_A, 0, RPN_B, 1,
                 4'h2, 2'b10);
      check(dut.tlb.valid_q[1][0][1] && dut.tlb.valid_q[1][1][1] &&
            dut.tlb.valid_q[0][0][1] && dut.tlb.valid_q[1][0][2],
            "fill lost a neighbor or bank entry");
      set_sr(4'd1, {8'h00, VSID_A});
      set_context(0, 1, 0);
      data_access(0, EA, 1, {RPN_A, 12'h234}, 4'h6);
      data_access(0, 32'h1000_2234, 1, {RPN_B, 12'h234}, 4'h2);
      set_context(1, 0, 0);
      ...
      // Response consumption and commit may occur on the same edge.
      fill_offer(1, 32'h1000_a234, VSID_A, 0, 20'ha1234,
                 1, 4'h5, 2'b10, 0);
      @(negedge clk_i);
      tlb_fill_rsp_ready_i = 1; tlb_fill_commit_i = 1;
      @(posedge clk_i); #1;
      ...
      // Held response captures every accepted field despite caller changes.
      fill_offer(1, 32'h1000_3234, VSID_A, 0, 20'hc1234,
                 1, 4'h7, 2'b10, 0);
      @(negedge clk_i);
      tlb_fill_req_bank_i = 0; tlb_fill_req_ea_i = ~EA;
      tlb_fill_req_vsid_i = VSID_B; tlb_fill_req_way_i = 1;
      tlb_fill_req_rpn_i = 20'hfffff; tlb_fill_req_c_i = 0;
      tlb_fill_req_wimg_i = 0; tlb_fill_req_pp_i = 0;
      tlb_mgmt_req_valid_i = 1; bat_csr_req_valid_i = 1;
      segment_csr_req_valid_i = 1; context_valid_i = 1;
      repeat (2) begin
        @(posedge clk_i); #1;
        check(tlb_fill_rsp_valid_o && !tlb_fill_rsp_error_o &&
              !tlb_mgmt_req_ready_o && !bat_csr_req_ready_o &&
              !segment_csr_req_ready_o && !context_ready_o &&
              !dmem_req_ready_o, "held fill response leaked ownership");
      end
      @(negedge clk_i);
      ...
      #1; check(!tlb_mgmt_req_ready_o && tlb_fill_ack_valid_o,
                "held fill ack lost reservation");
      tlb_mgmt_req_valid_i = 0;
      fill_ack_and_idle();

      // Abort while response is held leaves the selected way unmodified.
      fill_offer(1, 32'h1000_4234, VSID_A, 0, 20'hd1234,
                 1, 4'h1, 2'b10, 0);
      fill_abort_and_idle();
      check(!dut.tlb.valid_q[1][0][4], "aborted fill committed entry");

      // Router PR is the privilege source even when all fill fields are valid.
      set_context(0, 0, 1);
      fill_offer(1, 32'h1000_5234, VSID_A, 0, 20'he1234,
                 1, 4'h1, 2'b10, 1);
      fill_consume();
      @(posedge clk_i); #1;
      check(tlb_fill_idle_o && !dut.tlb.valid_q[1][0][5],
            "privileged fill created proposal");
      set_context(0, 0, 0);

      // CPU tlbie wins simultaneous fill and normalized management offers.
      @(negedge clk_i);
      tlb_inv_req_valid_i = 1; tlb_inv_req_ea_i = 32'h1001_f234;
      tlb_fill_req_valid_i = 1; tlb_fill_req_bank_i = 1;
      tlb_fill_req_ea_i = 32'h1000_6234;
      tlb_mgmt_req_valid_i = 1;
      ...
      @(negedge clk_i); tlb_inv_req_valid_i = 0;
      tlb_inv_abort_i = 1; tlb_inv_rsp_ready_i = 1;
      @(posedge clk_i); #1;
      check(!tlb_inv_rsp_valid_o && !tlb_fill_req_ready_o,
            "invalidate response did not drain");
      @(negedge clk_i); tlb_inv_abort_i = 0;
      tlb_inv_rsp_ready_i = 0;
      @(posedge clk_i); #1;
      ...
      tlb_mgmt_req_valid_i = 0;
      fill_abort_and_idle();
      manage(2'd0, 1, EA, VSID_A, 0, RPN_A, 1, 4'h1, 2'b10, 1);

      // Corrupt the service's held identity to verify error and abort drain.
      fill_offer(1, 32'h1000_7234, VSID_A, 0, 20'hf1234,
                 1, 4'h1, 2'b10, 0);
      @(negedge clk_i);
      force dut.tlb_rsp_ea = 32'hffff_ffff;
      #1; check(tlb_fill_rsp_error_o, "response identity mismatch unreported");
      release dut.tlb_rsp_ea;
      fill_abort_and_idle();
      check(!dut.tlb.valid_q[1][0][7], "identity error mutated TLB");

      // Reset must also discard a held response and a consumed proposal.
      fill_offer(1, 32'h1000_b234, VSID_A, 0, 20'hb1234,
                 1, 4'h1, 2'b10, 0);
      @(negedge clk_i); rst_ni = 0;
      #1; check(!tlb_fill_rsp_valid_o && !tlb_fill_idle_o,
                "reset did not mask held fill response");
      @(posedge clk_i);
      @(negedge clk_i); rst_ni = 1;
      #1; check(tlb_fill_idle_o && !dut.tlb.valid_q[1][0][11],
                "reset did not discard held response/proposal");
      start_router(0, 0, 0);
      fill_offer(1, 32'h1000_c234, VSID_A, 0, 20'hc1234,
                 1, 4'h1, 2'b10, 0);
      fill_consume();
      @(negedge clk_i); rst_ni = 0;
      #1; check(!tlb_fill_ack_valid_o && !tlb_fill_idle_o,
                "reset did not mask uncommitted proposal");
      @(posedge clk_i);
      @(negedge clk_i); rst_ni = 1;
      #1; check(tlb_fill_idle_o && !dut.tlb.valid_q[1][0][12],
                "reset did not discard consumed proposal");
      start_router(0, 0, 0);

      // Reset tears down a held committed ack and all local valid bits.
      fill_offer(1, 32'h1000_8234, VSID_A, 0, 20'hb1234,
                 1, 4'h1, 2'b10, 0);
      fill_consume(); fill_commit();
      @(negedge clk_i); rst_ni = 0;
      #1; check(!tlb_fill_ack_valid_o && !tlb_fill_idle_o &&
                !tlb_fill_rsp_valid_o && !tlb_fill_req_ready_o,
                "reset did not mask held fill ack");
      @(posedge clk_i);
      @(negedge clk_i); rst_ni = 1;
        ...
        for (int bank_idx = 0; bank_idx < 2; bank_idx++) begin
          for (int way_idx = 0; way_idx < 2; way_idx++) begin
            fill_entry(bank_idx[0],
              32'h3000_0234 | (set_idx << 12),
              way_idx[0] ? VSID_B : VSID_A, way_idx[0],
              20'h80000 + 20'(set_idx), 1, 4'h2, 2'b10);
          end
        end
      end
  ...
  initial begin #200000; $fatal(1, "runtime TLB fill router watchdog"); end
endmodule

...
module tb_compiled_page_miss_firmware;
  import ppc_pkg::*;
  ...
  logic page_config_o;
  int page_retires=0,page_fetches=0,alias_stores=0,tlbld_retires=0,tlbli_retires=0,seed_writes=0;
  int mode=0,tlbie_retires=0,terminal_diagnostics=0;
  string image_path;
  logic unused_tlb_mgmt;
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      if(tv&&tr)begin
        if(retired.illegal)begin
          ...
          check(retired.page_miss.ea==((mode==1)?32'h20008000:32'h10008000)&&
                ...
                !retired.page_miss.pr&&retired.page_miss.ir&&retired.page_miss.dr&&
                retired.page_miss.write==(mode==2),"retired miss snapshot differs from access context");
          if(mode==1)
            check(retired.pc==32'h20008000&&retired.fetch_fault==FETCH_PAGE_MISS&&
                  retired.data_fault==DATA_OK,"instruction miss retirement identity");
          else
            check(retired.pc==BASE+((mode==2)?32'h7008:32'h7000)&&
                  retired.insn==((mode==2)?32'h90830000:32'h80630000)&&
                  retired.fetch_fault==FETCH_OK&&retired.data_fault==DATA_PAGE_MISS,
                  "data miss retirement identity");
          terminal_diagnostics++;
        end else begin
          check(!retired.alignment_exception&&retired.fetch_fault==FETCH_OK&&
                retired.data_fault==DATA_OK&&retired.page_miss=='0,
                "ordinary retirement leaked miss metadata");
        end
        if(retired.insn[31:26]==31&&retired.insn[10:1]==306)begin
          check(!retired.illegal&&!retired.gpr_write&&!retired.update_write,"TLBIE has destination/fault");
          tlbie_retires++;
        end
        if(retired.insn[31:26]==31&&(retired.insn[10:1]==978||retired.insn[10:1]==1010))begin
          check(!cir&&!cdr&&!cpr&&!retired.illegal&&!retired.gpr_write&&!retired.update_write,"TLB load context/effects");
          if(retired.insn[10:1]==978)tlbld_retires++;else tlbli_retires++;
        end
        selector={retired.insn[15:11],retired.insn[20:16]};
        if(retired.insn[31:26]==31&&retired.insn[10:1]==467&&
           (selector==977||selector==981||selector==982))seed_writes++;
        ...
        if(!retired.illegal&&(retired.pc==32'h20000000||retired.pc==32'h20000004||
      ...
      if(halted&&mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(transitions==5&&last_context==3&&phase==1,"missing context phases");
        check(interrupts==1&&decrements==1,"missing events");
        check(bat_writes==4&&segment_writes==((mode==2)?7:6)&&segment_reads==0,"missing CPU register accesses");
        check(tlbld_retires==2&&tlbli_retires==2&&seed_writes==8&&alias_stores==2&&page_retires==8&&page_fetches>=8,"missing page effects");
        check(word_at(BASE+32'h8000)==32'h13579bdf&&word_at(BASE+32'h9000)==32'h2468ace0,"page physical data corrupted");
        check(tlbie_retires==2&&terminal_diagnostics==1,"missing terminal invalidation diagnostic");
        $display("PASS compiled page-miss result firmware: mode=%0d TLBIE=%0d diagnostics=%0d Ipage_retires=%0d Ipage_fetches=%0d SRwrites=%0d SRreads=%0d BATwrites=%0d EXT=%0d DEC=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",mode,tlbie_retires,terminal_diagnostics,page_retires,page_fetches,segment_writes,segment_reads,bat_writes,interrupts,decrements,retires,reads,writes,cycles);$finish;
      end
    end
  end
  ...
  initial begin
    ...
    // All translation entries must be installed by CPU instructions.

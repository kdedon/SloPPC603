// Compiled DSI repair/retry firmware through real BAT translation and committed MSR context.
// Physical byte RAM uses independent delayed/backpressured instruction/data ports.
/* verilator lint_off BLKSEQ */
module tb_compiled_dsi_firmware;
  import ppc_pkg::*;
  ...
  int transitions=0,alias_stores=0,dsi_loads=0,dsi_stores=0;
    ...
    if(!rst_n)begin
      ...
      if(running)begin
        ...
        if({cir,cdr}!=last_context)begin
          ...
          transitions++;check(transitions<=26,"extra context transition");last_context={cir,cdr};
        end
      end
      ...
      if(dv&&dr)begin
        ...
        if(dw)begin
          ...
          if(da==BASE+32'h8000)begin
            check(st==15&&alias_stores<2,"faulting store escaped or extra physical store");
            check(alias_stores==0 ? (!cdr&&wd==32'h13579bdf) :
                  (cdr&&wd==32'h2468ace0&&dsi_loads==8&&dsi_stores==4),"protected physical word changed");
      ...
      if(tv&&tr)begin
        ...
        if(retired.insn[31:26]==31&&selector>=528&&selector<=543&&retired.insn[10:1]==467)bat_writes++;
        if(retired.data_fault!=DATA_OK)begin
          check(retired.data_fault==DATA_DSI_PROTECTION&&!retired.gpr_write&&!retired.update_write&&!retired.write_xer,"DSI leaked architectural result");
          if(retired.insn[31:26]==33)dsi_loads++;
          else begin check(retired.insn[31:26]==37,"wrong fault instruction");dsi_stores++;end
        end

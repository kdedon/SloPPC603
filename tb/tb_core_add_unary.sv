// Full-core ADDME/ADDZE OE/Rc validation with an independent unary arithmetic oracle.
/* verilator lint_off BLKSEQ */
module tb_core_add_unary;
  ...
  int unary_form_commits [UNARY_FORMS];
  int unary_commits = 0;
  int addc_seed_commits = 0;
  int owner_release_exact = 0;
  int unary_after_seed_exact = 0;
  ...
  logic saw_addme_zero_ca0 = 0;
  logic saw_addme_min_ca0 = 0;
  logic saw_addme_min_ca1 = 0;
  logic saw_addme_ca1_identity = 0;
  logic saw_addze_max_ca1 = 0;
  logic saw_addze_ffff_ca1 = 0;
  ...
  function automatic logic [31:0] encode_unary(
    input logic addme, input logic oe, input logic rc,
    input logic [4:0] rd, input logic [4:0] ra, input logic [4:0] reserved_rb
  );
    logic [8:0] xo;
    xo = addme ? 9'd234 : 9'd202;
    return {6'd31, rd, ra, reserved_rb, oe, xo, rc};
  endfunction
  ...
  function automatic logic is_unary_xo(
    input logic [5:0] opcd, input logic [8:0] xo
  );
    return opcd == 6'd31 && (xo == 9'd234 || xo == 9'd202);
  endfunction

  function automatic logic is_legal_unary(
    input logic [5:0] opcd, input logic [8:0] xo,
    input logic [4:0] reserved_rb
  );
    return is_unary_xo(opcd, xo) && reserved_rb == 0;
  endfunction
  ...
  function automatic logic expected_legal(input logic [31:0] insn);
    ...
    return is_add_encoding(insn[31:26], insn[9:1]) ||
           is_legal_unary(insn[31:26], insn[9:1], insn[15:11]) ||
  ...
  function automatic logic expected_needs_flags(input logic [31:0] insn);
    ...
    if (is_legal_unary(insn[31:26], insn[9:1], insn[15:11])) return 1'b1;
  ...
  task automatic oracle_commit(input retire_packet_t packet,
    ...
    logic carry_in, write_ca, unary, addme;
    logic [3:0] cr0;
    logic addc, oe, rc;
    ...
    if (packet.illegal) begin
      ...
      if (is_unary_xo(packet.insn[31:26], packet.insn[9:1]) &&
          packet.insn[15:11] != 0)
        reserved_rejections++;
      return;
    end
    ...
    if (is_add_encoding(packet.insn[31:26], packet.insn[9:1]) ||
        is_legal_unary(packet.insn[31:26], packet.insn[9:1],
                       packet.insn[15:11])) begin
      addc = packet.insn[9:1] == 9'd10;
      unary = is_legal_unary(packet.insn[31:26], packet.insn[9:1],
                             packet.insn[15:11]);
      addme = packet.insn[9:1] == 9'd234;
      oe = packet.insn[10];
      rc = packet.insn[0];
      a = model_gpr[packet.insn[20:16]];
      b = unary ? (addme ? 32'hffff_ffff : 32'b0) :
                  model_gpr[packet.insn[15:11]];
      carry_in = unary ? old_ca : 1'b0;
      write_ca = addc || unary;
      ...
      if (unary)
        signed_total = signed_a + (addme ? -64'sd1 : 64'sd0) +
                       $signed({63'b0, carry_in});
      else
      ...
      require(packet.gpr_write && packet.gpr == packet.insn[25:21] &&
              packet.value == value,
              "ADD/addc/unary GPR result or destination mismatch");
      require(packet.needs_flags == (write_ca || oe || rc) &&
              packet.write_ca == write_ca && packet.write_ov_so == oe &&
              packet.write_cr0 == rc,
              "ADD/addc/unary allocated flag permissions mismatch");
      require(packet.xer_delta == {
              ...
              "ADD/addc/unary masked flag delta mismatch");
      ...
      if (unary) begin
        unary_form_commits[form]++;
        unary_commits++;
      end else if (addc) begin
        addc_seed_commits++;
      end

      if (unary && addme && a == 0 && !carry_in &&
          value == 32'hffff_ffff && !carry && !overflow)
        saw_addme_zero_ca0 = 1;
      if (unary && addme && a == 32'h8000_0000 && !carry_in &&
          value == 32'h7fff_ffff && carry && overflow)
        saw_addme_min_ca0 = 1;
      if (unary && addme && carry_in && value == a && carry)
        saw_addme_ca1_identity = 1;
      if (unary && addme && a == 32'h8000_0000 && carry_in &&
          value == 32'h8000_0000 && carry && !overflow)
        saw_addme_min_ca1 = 1;
      if (unary && !addme && a == 32'h7fff_ffff && carry_in &&
          value == 32'h8000_0000 && !carry && overflow)
        saw_addze_max_ca1 = 1;
      if (unary && !addme && a == 32'hffff_ffff && carry_in &&
          value == 0 && carry && !overflow)
        saw_addze_ffff_ca1 = 1;
      if (oe && old_so && !overflow && !new_ov && new_so) saw_sticky_nonoverflow = 1;
      if (unary && carry_in && carry == new_ca) saw_ca_chain = 1;
      if (unary && !carry_in) saw_ca0_capture = 1;
      if (!oe && (old_ov || old_so) && model_xer[31:30] == old_xer[31:30])
        saw_oe0_preservation = 1;
      if (oe && rc && !old_so && overflow && cr0[0]) saw_rc_final_so = 1;
      if (unary && !rc && model_cr == old_cr) saw_rc_preservation = 1;
      if (packet.gpr == 0) r0_writes++;
      if (packet.insn[20:16] == 0 || (!unary && packet.insn[15:11] == 0))
        r0_reads++;
  ...
  always @(posedge clk) begin
    ...
    if (!rst_n) begin
      ...
      if (int'(dut.iq.count) == IQ_DEPTH &&
        ...
        require(!req_valid, "full unary ADD IQ admitted an unreserved fetch");
        credit_stalls++;
      end
  ...
  initial begin
    ...
    for (int form = 0; form < UNARY_FORMS; form++) unary_form_commits[form] = 0;
    clear_program();
    require(encode_unary(1, 0, 0, 5'd7, 5'd13, 0) == 32'h7ced_01d4,
            "ADDME rD/rA/reserved-rB encoding anchor");
    require(encode_unary(0, 1, 1, 5'd7, 5'd13, 0) == 32'h7ced_0595,
            "addzeo. OE/Rc/XO encoding anchor");

    // Setup exact boundary operands.
    append_addi(5'd1, 0, 16'h0000);              // 00000000
    append_addi(5'd2, 0, 16'h0001);              // 00000001
    append_addi(5'd3, 0, 16'hffff);              // ffffffff
    append_addis(5'd4, 0, 16'h7fff);
    append_ori(5'd4, 5'd4, 16'hffff);             // 7fffffff
    append_addis(5'd5, 0, 16'h8000);              // 80000000

    // Every directed CA is established by a committed ADDC in the actual core.
    append(encode_add(1, 0, 0, 5'd10, 5'd1, 5'd1)); // seed CA=0
    append(encode_unary(1, 0, 0, 5'd11, 5'd1, 0));   // addme 0,CA0 -> ffffffff
    append(encode_add(1, 0, 0, 5'd12, 5'd1, 5'd1)); // seed CA=0
    append(encode_unary(1, 1, 1, 5'd13, 5'd5, 0));   // min-1 -> max, OV/SO
    append(encode_add(1, 0, 0, 5'd14, 5'd3, 5'd2)); // seed CA=1, preserve SO
    append(encode_unary(1, 1, 0, 5'd15, 5'd5, 0));   // min identity, no OV

    append(encode_add(1, 0, 0, 5'd16, 5'd3, 5'd2)); // seed CA=1
    append(encode_unary(0, 1, 1, 5'd17, 5'd4, 0));   // max+1 -> min, OV
    append(encode_add(1, 0, 0, 5'd18, 5'd3, 5'd2)); // seed CA=1
    append(encode_unary(0, 0, 1, 5'd19, 5'd3, 0));   // ffffffff+1 -> 0, CA1
    append(encode_add(1, 0, 0, 5'd20, 5'd3, 5'd2)); // seed CA=1
    append(encode_unary(0, 1, 0, 5'd21, 5'd3, 0));   // clear OV, sticky SO

    append(encode_add(1, 0, 0, 5'd22, 5'd3, 5'd2)); // seed CA=1
    append(encode_unary(1, 0, 1, 5'd23, 5'd1, 0));   // addme identity zero
    append(encode_add(1, 0, 0, 5'd24, 5'd1, 5'd1)); // seed CA=0
    append(encode_unary(0, 0, 0, 5'd25, 5'd1, 0));   // addze zero,CA0

    // r0 and rD=rA are real unary dependencies with immediate RAW/WAW reuse.
    append(encode_add(1, 0, 0, 5'd0, 5'd3, 5'd2));
    append(encode_unary(1, 0, 0, 5'd0, 5'd0, 0));
    append(encode_unary(0, 0, 1, 5'd0, 5'd0, 0));

    // Deterministic mixed RAW/WAW pressure. Form index cycles through all eight.
    random_state = 32'h603e_a11e;
    for (int i = 0; i < 64; i++) begin
      ...
      ra = random_state[9:5];
      rd = (i % 13 == 0) ? 0 :
           ((i % 5 == 0) ? ra : 5'(5 + (int'(random_state[4:0]) % 20)));
      append(encode_unary(addme, oe, rc, rd, ra, 0));
    end
    append(32'b0);
    ...
    require(phase_retirements == main_words,
            "unary ADD main program retirement count mismatch");
    for (int form = 0; form < UNARY_FORMS; form++)
      require(unary_form_commits[form] > 0,
              "unary ADD corpus missed a family/OE/Rc form");
    require(saw_addme_zero_ca0 && saw_addme_min_ca0 && saw_addme_min_ca1 &&
            saw_addme_ca1_identity && saw_addze_max_ca1 &&
            saw_addze_ffff_ca1 && saw_ca0_capture,
            "unary ADD corpus missed a signed/unsigned/CA boundary");
    require(saw_sticky_nonoverflow && saw_ca_chain && saw_oe0_preservation &&
            saw_rc_final_so && saw_rc_preservation,
            "unary ADD corpus missed carry/sticky/preservation/final-SO checks");
    require(r0_writes >= 2 && r0_reads >= 2,
            "unary ADD corpus missed explicit r0/rD=rA RAW/WAW coverage");
    require(owner_release_exact > 0,
            "unary ADD corpus did not observe owner admission on commit+1");
    require(unary_after_seed_exact > 0,
            "unary ADD never captured CA on the cycle after a seed committed");
    require(request_stalls > 0 && credit_stalls > 0 && retirement_stalls > 0,
            "unary ADD corpus missed request, full-IQ credit, or retirement stalls");

    // Both nonzero reserved-rB probes reject for every family/OE/Rc form.
    for (int family = 0; family < 2; family++) begin
      for (int flags = 0; flags < 4; flags++) begin
        for (int rb_choice = 0; rb_choice < 2; rb_choice++) begin
          clear_program();
          append(encode_unary(family != 0, flags[1], flags[0], 5'd7, 5'd13,
                              rb_choice == 0 ? 5'd1 : 5'd31));
          reset_core();
          retire_enable = 1;
          await_halt();
          require(phase_retirements == 1 && model_cr == 0 && model_xer == 0 &&
                  !dut.flags_busy,
                  "reserved unary rB did not retire as a clean diagnostic");
        end
      end
    end
    require(reserved_rejections == 16,
            "unary ADD corpus did not observe all 16 reserved-rB rejections");

    // XO137 remains unsupported; SUBFE now has its own execution/recovery suite.
    clear_program();
    append({6'd31, 5'd7, 5'd13, 5'd29, 1'b1, 9'd137, 1'b1});
    reset_core();
    retire_enable = 1;
    await_halt();
    require(phase_retirements == 1 && model_cr == 0 && model_xer == 0 &&
            !dut.flags_busy,
            "unsupported XO137 did not retire as one clean diagnostic");

    $display("PASS core unary ADD: checks=%0d retire=%0d unary=%0d seeds=%0d forms=8 reserved=16 owner+1=%0d seed+1=%0d",
             checks, total_retirements, unary_commits, addc_seed_commits,
             owner_release_exact, unary_after_seed_exact);
    $finish;
  end

  initial begin
    #2000000;
    $fatal(1, "core unary ADD watchdog at edge %0d", edge_count);
  end
endmodule

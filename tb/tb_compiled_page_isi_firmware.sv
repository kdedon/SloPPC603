...
module tb_compiled_page_isi_firmware;
  import ppc_pkg::*;
  ...
  int transitions=0,isi_pp=0,isi_guarded=0,tlbli_retires=0,page_fetches=0,segment_writes=0;
  ...
  ppc_core_bat #(.ENABLE_SUPERVISOR_EXCEPTIONS(1'b1),.ENABLE_LIVE_CONTEXT(1'b1),
                 ...
                 .ENABLE_PAGE_INSTRUCTION_EXCEPTIONS(1'b1),.ENABLE_TLB_LOAD(1'b1)) dut(
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      cycles++;check(cycles<100000,"page ISI firmware timeout");
      ...
      if(fault)check(unused_fi&&!unused_fp&&!unused_fm&&!unused_fg&&!unused_fc&&
                     !unused_finv&&unused_fentries==0&&unused_fea[31:12]==20'h20000&&
                     unused_page_ports[42]&&!unused_page_ports[43],
      ...
      if(running)begin
        check(!bat_ready&&!cpr&&!cdr,"data translation unexpectedly enabled");
        if(cir!=last_context)begin
          transitions++;last_context=cir;
          check(transitions<=14,"extra IR context transition");
        end
      end
      ...
      if(iv&&ir)begin
        ...
        if(ia==BASE+32'h6000&&cir)begin
          case(page_fetches)
            0: check(tlbli_retires>=2,"PP-denied fetch reached physical probe before TLBLI repair");
            1: check(segment_writes>=3,"N-denied fetch reached physical probe before SR repair");
            2: check(tlbli_retires>=4,"guarded fetch reached physical probe before TLBLI repair");
            default: check(0,"extra translated physical probe fetch");
          endcase
          page_fetches++;
        end
        ipending<=1;fetch_pc<=ia;idelay<=1+cycles%4;
        if(ia==BASE+32'h6000)idelay<=13;
      end
      if(dv&&dr)begin
        check(da>=BASE&&da<=BASE+32'hfffc&&da[1:0]==0,"data physical RAM range");
        check(dwimg==4'b0011,"real-mode data attributes");
      ...
      if(tv&&tr)begin
        check(!retired.illegal&&!retired.alignment_exception&&retired.data_fault==DATA_OK,
        ...
        if(retired.insn[31:26]==31&&
           (retired.insn[10:1]==210||retired.insn[10:1]==242))segment_writes++;
        if(retired.insn[31:26]==31&&retired.insn[10:1]==1010)begin
          check(!cir&&!cdr&&!retired.illegal&&!retired.gpr_write&&!retired.update_write,
                "TLBLI context or side effect");
          tlbli_retires++;
        end
        if(retired.fetch_fault!=FETCH_OK)begin
          check(retired.pc==32'h20000000&&!retired.gpr_write&&!retired.update_write&&
                !retired.write_xer,"ISI escaped precise fault boundary");
          if(retired.fetch_fault==FETCH_ISI_PROTECTION)begin
            check(isi_pp==0&&isi_guarded==0,"unexpected PP fault order");isi_pp++;
          end else begin
            check(retired.fetch_fault==FETCH_ISI_GUARDED&&isi_guarded<2&&isi_pp==1,
                  "unexpected N/G fault order");isi_guarded++;
          end
        end
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(transitions==12&&!cir&&fault&&unused_page_ports[42]&&
              unused_page_ports[44]&&unused_page_ports[45]&&unused_page_ports[46]&&
              !unused_page_ports[43]&&!unused_page_ports[47]&&
              !unused_page_ports[48]&&!unused_page_ports[49],
              "missing clean page PP/N/G diagnostics or context transitions");
        check(isi_pp==1&&isi_guarded==2&&bat_writes==12&&tlbli_retires==4&&
              segment_writes==3&&
              page_fetches>=3,"missing CPU-seeded page ISI effects");
        $display("PASS compiled page ISI firmware: PP=%0d guarded=%0d TLBLI=%0d BATwrites=%0d retires=%0d cycles=%0d",
                 isi_pp,isi_guarded,tlbli_retires,bat_writes,retires,cycles);
        $finish;
      end
    end
  end

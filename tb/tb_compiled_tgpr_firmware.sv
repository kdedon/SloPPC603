...
module tb_compiled_tgpr_firmware;
  import ppc_pkg::*;
  ...
  ppc_core_bat #(.ENABLE_SUPERVISOR_EXCEPTIONS(1'b1),
    .ENABLE_LIVE_CONTEXT(1'b1),.ENABLE_TGPR(1'b1)) dut(
  ...
  always @(posedge clk)begin : monitor
    if(!rst_n)begin
      ...
      cycles++;check(cycles<20000,"TGPR firmware timeout");
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(mtmsr_retires==3&&rfi_retires==2&&bank_switches==4&&!last_bank,
              "missing TGPR bank transitions or RFI");
        $display("PASS compiled TGPR firmware: switches=%0d MTMSR=%0d RFI=%0d retires=%0d cycles=%0d",
                 bank_switches,mtmsr_retires,rfi_retires,retires,cycles);$finish;
      end
    end
  end

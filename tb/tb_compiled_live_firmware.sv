// Compiled firmware through real BAT translation and committed live MSR context.
// Physical byte RAM uses independent delayed/backpressured instruction/data ports.
/* verilator lint_off BLKSEQ */
module tb_compiled_live_firmware;
  import ppc_pkg::*;
    ...
    if(!rst_n)begin
      ...
      if(running)begin
        ...
        if({cir,cdr}!=last_context)begin
          ...
          transitions++;check(transitions<=4,"extra context transition");last_context={cir,cdr};
        end
      end
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(transitions==4&&last_context==0,"missing context transitions");
        check(alias_stores==1&&reads>0&&writes>0,"missing mapped memory activity");
        $display("PASS compiled live firmware: transitions=%0d alias_stores=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",transitions,alias_stores,retires,reads,writes,cycles);$finish;
      end
    end
  end

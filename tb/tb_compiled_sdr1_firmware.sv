...
module tb_compiled_sdr1_firmware;
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      cycles++;check(cycles<20000,"SDR1 firmware timeout");
      check(!halted&&!cut_accepted&&!fault&&!physical_error&&
      ...
      if(running)check(!bat_ready&&!cpr&&!cir&&!cdr,"real supervisor context changed");
      ...
      if(tv&&tr)begin
        check(!retired.illegal&&!retired.alignment_exception&&
          ...
          retired.page_miss=='0,"retirement diagnostic");
        selector={retired.insn[15:11],retired.insn[20:16]};
        if(retired.insn[31:26]==31&&selector==25)begin
          if(retired.insn[10:1]==467)begin
            check(!retired.gpr_write&&!retired.update_write,"SDR1 write granted GPR effects");
            sdr1_writes++;
          end else if(retired.insn[10:1]==339)begin
            check(retired.gpr_write&&sdr1_reads<5&&retired.value==expected_read(sdr1_reads),
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(sdr1_writes==4&&sdr1_reads==5,"missing SDR1 roundtrips");
        $display("PASS compiled SDR1 firmware: writes=%0d reads=%0d retires=%0d cycles=%0d",
                 sdr1_writes,sdr1_reads,retires,cycles);$finish;
      end
    end
  end

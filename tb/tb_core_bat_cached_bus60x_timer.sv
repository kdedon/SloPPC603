...
module tb_core_bat_cached_bus60x_timer;
  import ppc_pkg::*;
  logic clk=0,rst_n=0;
  always #5 clk=~clk;
  ...
  logic pimem_error,irq=0,taken,dec_taken,tick=0;
  ...
  int cycles=0,checks=0,retires=0,alias_lines=0,irq_count=0,dec_count=0,ticks=0;
  int old_beats=0,ext_handler_retires=0,dec_handler_retires=0,ext_rfi_retires=0,dec_rfi_retires=0,target_retires=0,alias_hits=0;
  bit held_refill=0,release_refill=0,refill_drained=0,dec_reserved=0;
  ...
  ppc_core_bat_cached_bus60x #(.RESET_PC(32'b0),
    ...
    .timer_tick_i(tick),.timebase_enable_i(1'b0),
  ...
  always @(posedge clk)begin
    ...
    if(rst_n)begin
      ...
      if(cache_hit&&dec_rfi_retires>0)begin
        check((dut.imem_req_addr&32'hffffffe0)==32'h100,
          "post-RFI cache hit belongs to translated alias line");
        alias_hits++;
      end
      if(tick)ticks++;
      check(!(taken&&dec_taken),"mutually exclusive timer/EXT traces");
      if(dut.translated_core.core.special.interrupt_q &&
         dut.translated_core.core.special.decrementer_selected_q &&
         dut.translated_core.core.frontend_fence && !irq)dec_reserved=1;
      if(taken)begin
        check(irq&&irq_count==0&&dec_count==0&&dec_reserved&&
              refill_drained&&old_beats==4&&irq_pc==TARGET&&!tv&&
              dut.translated_core.core.decrementer_pending,
              "promoted EXT after old refill drain; DEC remains pending");
        irq_count++;
        irq<=0;
      end else check(irq_pc==0,"IRQ PC outside acceptance pulse");
      if(dec_taken)begin
        check(!irq&&irq_count==1&&dec_count==0&&dec_pc==TARGET&&
              ext_rfi_retires==1&&target_retires==0&&!tv&&
              dut.translated_core.core.decrementer_pending,
              "pending DEC accepted after external RFI at same target");
        dec_count++;
      end else check(dec_pc==0,"DEC PC outside acceptance pulse");
      if(tv&&tr)begin
        ...
        if(retired.pc==32'h900)begin
          check(dec_count==1&&!cir&&!cdr&&!cpr&&retired.insn==spr(0,22,26)&&
                retired.gpr_write&&retired.gpr==22&&retired.value==TARGET,
                "DEC handler observes exact SRR0");
          dec_handler_retires++;
        end
        if(retired.pc==32'h904)begin
          check(dec_count==1&&!cir&&!cdr&&!cpr&&retired.insn==spr(0,23,27)&&
                retired.gpr_write&&retired.gpr==23&&retired.value==32'h8020,
                "DEC handler observes EE/IR in SRR1");
          dec_handler_retires++;
        end
        if(retired.pc==32'h908)begin
          check(dec_count==1&&!cir&&!cdr&&!cpr&&retired.insn==RFI,
                "DEC handler RFI identity");
          dec_rfi_retires++;
        end
        if(retired.pc==TARGET)begin
          check(irq_count==1&&dec_count==1&&ext_rfi_retires==1&&dec_rfi_retires==1&&retired.insn==TARGET_INSN&&
  ...
  initial begin
    ...
    // CPU installs both IBATs, writes DEC=0, enables EE|IR, then branches
    // to the translated alias. The testbench never preloads mappings.
    ...
    put_word(24,spr(1,4,530)); put_word(28,32'h38c00000);
    put_word(32,spr(1,6,22)); put_word(36,32'h60058020);
    put_word(40,32'h7ca00124); put_word(44,32'h4c00012c);
    put_word(48,32'h3ce01000); put_word(52,32'h60e70100);
    put_word(56,spr(1,7,9)); put_word(60,32'h4e800420);
    put_word('h100,TARGET_INSN); put_word('h104,32'h48000000);
    put_word('h500,spr(0,20,26));put_word('h504,spr(0,21,27));
    put_word('h508,RFI);
    put_word('h900,spr(0,22,26));put_word('h904,spr(0,23,27));
    put_word('h908,RFI);
    ...
    // One sampled edge crosses DEC 0 -> -1 while the accepted alias line
    // remains held. A readonly reservation oracle fixes the EXT assertion
    // after DEC admission and before the frontend drain can complete.
    tick=1;
    @(negedge clk);tick=0;
    check(ticks==1&&dut.translated_core.core.decrementer_pending,
      "one public timer tick made DEC pending");
    for(int i=0;i<300&&!dec_reserved;i++)@(negedge clk);
    check(dec_reserved&&dut.translated_core.core.frontend_fence&&
          !dut.translated_core.core.frontend_quiescent&&
          dut.translated_core.core.special.decrementer_selected_q&&
          old_beats==0&&!taken&&!dec_taken,
          "DEC reserved while old cached refill blocks frontend drain");
    irq=1;
    repeat(8)begin
      @(posedge clk);#1;
      check(!taken&&!dec_taken&&irq_count==0&&dec_count==0&&
            old_beats==0&&target_retires==0,
            "both causes wait for accepted old response");
    end
    ...
    check(irq_count==1&&dec_count==0&&target_retires==0&&
          dut.translated_core.core.decrementer_pending,
          "EXT promoted at drained offer; DEC remains independently pending");
    for(int i=0;i<200&&!tv;i++)@(negedge clk);
    check(tv&&retired.pc==32'h500&&retired.insn==spr(0,20,26),
      "external handler retirement offered under backpressure");
    retire_enable=0;
    stalled=retired;
    repeat(4)begin
      @(posedge clk);#1;
      check(tv&&!tr&&retired===stalled&&target_retires==0&&
            ext_rfi_retires==0,"backpressure holds actual external handler offer");
    end
    @(negedge clk);retire_enable=1;
    for(int i=0;i<800&&target_retires==0;i++)@(negedge clk);
    check(target_retires==1&&ext_handler_retires==2&&dec_handler_retires==2&&
          ext_rfi_retires==1&&dec_rfi_retires==1&&irq_count==1&&dec_count==1&&
          !dut.translated_core.core.decrementer_pending&&
          alias_lines==1&&alias_hits>0&&ticks==1&&cir&&!cdr&&!cpr,
          "two precise handlers resume one translated cached target");
    repeat(20)@(negedge clk);
    check(target_retires==1&&irq_count==1&&dec_count==1&&alias_lines==1,
      "no duplicate target retirement or second alias refill");
    $display("PASS translated I-cache timer promotion: checks=%0d retires=%0d alias_lines=%0d old_beats=%0d EXT=%0d DEC=%0d ext_handler=%0d dec_handler=%0d target=%0d alias_hits=%0d",
      checks,retires,alias_lines,old_beats,irq_count,dec_count,
      ext_handler_retires,dec_handler_retires,target_retires,alias_hits);
    $finish;
  end
endmodule

...
module tb_core_bat_bus60x;
  ...
  function automatic logic [31:0] insn_at(input int a);
    case(a)
      0:return imm(14,3,0,2);             // r3 = valid/PP BAT bits
      4:return spr(1,3,529);              // IBAT0L
      8:return spr(1,3,528);              // IBAT0U identity
      12:return spr(1,3,539);             // DBAT1L
      16:return imm(15,4,0,'h1000);       // r4 = 0x10000000
      ...
      48:return imm(24,1,1,'h1000);       // r1 = EA 0x10001000
      52:return imm(14,2,0,'h5a);         // store byte value
      56:return imm(38,2,1,3);            // stb r2,3(r1)
      60:return imm(40,6,1,2);            // lhz r6,2(r1)
      64:return imm(32,7,1,0);            // lwz r7,0(r1)
      68:return imm(36,7,1,4);            // stw r7,4(r1)
      72:return 32'h44000002;             // sc
      76:return imm(14,8,0,9);            // after RFI
      80:return 32'h48000000;             // terminal self branch
  ...
  function automatic logic [31:0] expected_pc(input int ordinal);
    if(ordinal<=18)return 32'(ordinal*4);
    if(ordinal==19)return 32'hc00;
    if(ordinal==20)return 32'hc04;
    return 32'd76;
  endfunction
  ...
  always @(posedge clk)begin
    ...
    if(rst_n)begin
      ...
      if(tv&&tr)begin
        check(retires<22&&retired.pc==expected_pc(retires)&&
  ...
  initial begin
    for(int a=0;a<=80;a+=4)put_word(a,insn_at(a));
  ...
  initial begin
    ...
    check(retires==22&&fetch_real>0&&fetch_translated>0&&
          byte_writes==1&&half_reads==1&&word_reads==1&&word_writes==1&&
          data_reads==2&&data_writes==2&&retire_stalls>0,
    ...
    $display("PASS translated 60x directed: retires=%0d address=%0d fetch-real=%0d fetch-IR=%0d data-read=%0d data-write=%0d checks=%0d",

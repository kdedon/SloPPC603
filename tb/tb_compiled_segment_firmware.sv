...
module tb_compiled_segment_firmware;
  import ppc_pkg::*;
  ...
  ppc_core_bat #(.ENABLE_SUPERVISOR_EXCEPTIONS(1'b1),.ENABLE_LIVE_CONTEXT(1'b1),
                 .ENABLE_EXTERNAL_INTERRUPTS(1'b1),.ENABLE_TIMERS(1'b1),.ENABLE_RUNTIME_BAT(1'b1),.ENABLE_SEGMENT_REGISTERS(1'b1)) dut(
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      if(interrupt_taken)begin
        check(external_irq&&interrupts==0&&decrements==0&&bat_writes==4&&segment_writes==36,"IRQ ordering or lost pending level");
      ...
      if(decrementer_taken)begin
        check(interrupts==1&&decrements==0&&!interrupt_taken&&bat_writes==4&&segment_writes==36,"DEC ordering");
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        ...
        check(bat_writes==4&&segment_writes==36&&segment_reads==38,"missing CPU register accesses");
        $display("PASS compiled segment firmware: SRwrites=%0d SRreads=%0d BATwrites=%0d EXT=%0d DEC=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",segment_writes,segment_reads,bat_writes,interrupts,decrements,retires,reads,writes,cycles);$finish;
      end
    end
  end

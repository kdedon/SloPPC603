// CR transfers: architectural preservation, cancellation and retirement cuts.
/* verilator lint_off BLKSEQ */
module tb_core_crtransfer_edges;
  ...
  function automatic logic [31:0] instruction(input logic [31:0] pc);
    case(pc)
      ...
      28:return kind==0 ? 32'h7c800026 : 32'h7cca5120;
      ...
      if(tv && tr) begin
        ...
        case(expected_pc)
          ...
          default:require(retired.illegal && !retired.gpr_write && !retired.needs_flags &&
                          !retired.write_cr_fields,"diagnostic has effects");
        endcase
        retires++;
  ...
  initial begin
    ...
    $display("PASS CR transfer edges: before-finish kill, held head, kept finish/commit, CR/XER/GPR preservation (%0d checks)",checks);
    $finish;
  end
  initial begin #200000;$fatal(1,"CR transfer edge watchdog");end
endmodule

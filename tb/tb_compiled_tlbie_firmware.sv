...
module tb_compiled_tlbie_firmware;
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      if(halted&&mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        ...
        check(preloads==4&&alias_stores==2&&page_retires==8&&page_fetches>=8,"missing page effects");
        ...
        $display("PASS compiled TLBIE firmware: mode=%0d TLBIE=%0d diagnostics=%0d Ipage_retires=%0d Ipage_fetches=%0d SRwrites=%0d SRreads=%0d BATwrites=%0d EXT=%0d DEC=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",mode,tlbie_retires,terminal_diagnostics,page_retires,page_fetches,segment_writes,segment_reads,bat_writes,interrupts,decrements,retires,reads,writes,cycles);$finish;
      end
    end
  end
  ...
  initial begin
    ...
    preload(0,32'h20008000,24'h005678,0,20'hfff06);

// Compiled IRQ firmware through real BAT translation and committed MSR context.
// Physical byte RAM uses independent delayed/backpressured instruction/data ports.
/* verilator lint_off BLKSEQ */
module tb_compiled_irq_firmware;
  import ppc_pkg::*;
  ...
  int transitions=0,alias_stores=0,interrupts=0;
  logic external_irq,interrupt_taken,second_irq_pending=0,alias_retired=0;
  ...
  always @(posedge clk)begin
    if(!rst_n)begin
      ...
      if(dv&&dr)begin
        ...
        if(dw)begin
          ...
          if(da==BASE+32'h3000&&wd==32'h13579bdf)begin
            ...
            check(interrupts==1&&!second_irq_pending,"alias store IRQ ordering/duplicate");
            alias_stores++;second_irq_pending=1;ddelay<=24;
          end
      ...
      if(interrupt_taken)begin
        check(external_irq&&interrupts<2,"unexpected interrupt trace");
        ...
        if(interrupts==1)check(alias_retired&&!dpending,"IRQ overtook store completion");
        interrupts++;
      end
      if(tv&&tr)begin
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        check(transitions==6&&last_context==0,"missing context transitions");
        check(alias_stores==1&&alias_retired&&reads>0&&writes>0,"missing mapped memory activity");
        check(interrupts==2,"expected exactly two interrupts");
        $display("PASS compiled IRQ firmware: interrupts=%0d transitions=%0d alias_stores=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",interrupts,transitions,alias_stores,retires,reads,writes,cycles);$finish;
      end
    end
  end

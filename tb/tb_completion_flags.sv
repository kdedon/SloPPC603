...
module tb_completion_flags;
  ...
  assign flags_alloc_needs = !allocation.illegal &&
    (allocation.needs_flags || allocation.write_ca ||
     allocation.write_ov_so || allocation.write_cr0 || allocation.write_xer);
  ...
  task automatic allocate_packet(
    ...
    effective_needs = !packet.illegal &&
      (packet.needs_flags || packet.write_ca ||
       packet.write_ov_so || packet.write_cr0 || packet.write_xer);
  ...
  initial begin
    ...
    require(!retired.needs_flags && !retired.write_ca &&
            !retired.write_ov_so && !retired.write_cr0 && !retired.write_xer,
    ...
    require(!retired.gpr_write && !retired.needs_flags &&
            !retired.write_ca && !retired.write_ov_so && !retired.write_cr0 && !retired.write_xer &&
    ...
    // Full XER permission originates at allocation; result value supplies the
    // defined bits, independently of the arithmetic candidates in the result.
    packet = make_allocation(18, 0, 0, 0, 0, 0);
    packet.gpr_write = 0; packet.write_xer = 1;
    allocate_packet(packet, owner_tag);
    held_xer = xer;
    send_result(owner_tag, 32'hffffffff, 0, 0, 0, 0, 1, 0);
    require(retire_valid && retired.needs_flags && retired.write_xer &&
            !retired.gpr_write && retired.xer_delta == 32'he000007f,
            "full XER allocation/result mask wrong");
    repeat (3) begin
      @(posedge clk); #1;
      require(xer == held_xer, "full XER mutated before retirement");
    end
    accept_retirement();
    require(xer == 32'he000007f, "full XER commit omitted byte count");
    packet = make_allocation(19, 0, 0, 0, 0, 0);
    packet.gpr_write = 0; packet.write_xer = 1;
    allocate_packet(packet, owner_tag);
    send_result(owner_tag, 32'h40000035, 1, 1, 1, 15, 1, 0);
    require(retired.xer_delta == 32'h40000035, "MTXER incorrectly applied sticky SO semantics");
    accept_retirement();
    require(xer == 32'h40000035, "MTXER failed to clear SO or preserve exact byte count");
    ...
    $display("PASS completion flags: sanitization, identity, masks, recovery, fill/wrap (%0d checks)", checks);
    $finish;
  end

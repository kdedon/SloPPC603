...
module tb_compiled_table_bus60x_firmware #(
    ...
    if(!rst_n)begin
      ...
      if(ts_oe&&!ts_n)begin
        ...
        check(a>=BASE&&a<=BASE+32'h2fffc&&
          ...
          int'(a-BASE)+int'(tsiz)<=196608,
          "60x RAM address/size");
        check(tbst_n&&!ci_n&&wt_n&&gbl_n&&cse==0&&
          (tc==0||tc==2)&&
          (tt==5'b01010||tt==5'b00010),
  ...
  always @(posedge clk) begin : oracle
    ...
    if(!rst_n)begin
      ...
      cycles++;
      check(cycles<(FAULT_PROFILE==0?1500000:8000000), $sformatf("bus firmware timeout retire=%0d searchmiss=%0d faultmiss=%0d pinreads=%0d pinwrites=%0d mailbox=%b/%b busy=%b/%b corepc=%08x corestate=%0d",retires,search_misses,fault_misses,pin_reads,pin_writes,mailbox_written,mailbox_retired,router_busy,bus_busy,dut.translated_core.core.iq_head.pc,dut.translated_core.core.special.state_q));
      ...
      if(mailbox_retired)begin
        if(FAULT_PROFILE==0)begin
          ...
          $display("PASS compiled table search 60x: misses=%0d pinreads=%0d pinwrites=%0d ifetch=%0d retires=%0d cycles=%0d checks=%0d",
            search_misses,pin_reads,pin_writes,pin_ifetches,retires,cycles,checks);
        end else begin
          ...
          $display("PASS compiled table fault 60x: cases=%0d misses=%0d vectors=%0d fills=%0d pinreads=%0d pinwrites=%0d retires=%0d cycles=%0d checks=%0d",
            fault_started,fault_misses,fault_vectors,fault_fills,
            pin_reads,pin_writes,retires,cycles,checks);
        end
        $finish;
      end
    end

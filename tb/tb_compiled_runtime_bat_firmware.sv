// Compiled runtime BAT firmware through real BAT translation and committed MSR context.
// Physical byte RAM uses independent delayed/backpressured instruction/data ports.
/* verilator lint_off BLKSEQ */
module tb_compiled_runtime_bat_firmware;
  import ppc_pkg::*;
  ...
  int bat_reads=0,bat_writes=0,alias_retirements=0;
  ...
  always @(posedge clk)begin : monitor
    ...
    if(!rst_n)begin
      ...
      if(dv&&dr)begin
        ...
        if(dw)begin
          writes++;
          if(da==BASE+32'h8000||da==BASE+32'h28000)begin
            check(cdr&&st==15&&alias_stores<2,"alias store context/count");
            check(alias_stores==0 ? (da==BASE+32'h8000&&wd==32'h13579bdf) :
                  (da==BASE+32'h28000&&wd==32'h2468ace0),"replacement physical mapping");
            if(alias_stores==1)check(interrupts==1&&decrements==1,"pending events lost across BAT replacement");
            alias_stores++;ddelay<=18;
          end
          for(int lane=0;lane<4;lane++)if(st[3-lane])mem[int'(da-BASE)+lane]=wd[31-lane*8 -:8];
          if(da==BASE+32'h8004)begin
      ...
      if(interrupt_taken)begin
        check(external_irq&&interrupts==0&&decrements==0&&bat_writes==25,"IRQ ordering or lost pending level");
      ...
      if(decrementer_taken)begin
        check(interrupts==1&&decrements==0&&!interrupt_taken&&bat_writes==25,"DEC ordering");
      ...
      if(tv&&tr)begin
        ...
        if(retired.insn[31:26]==31&&selector>=528&&selector<=543)begin
          ...
          else if(retired.insn[10:1]==339||retired.insn[10:1]==371)bat_reads++;
        end
        if(alias_retirements<alias_stores)begin
          check(retired.insn[31:26]==36,"alias store retirement");alias_retirements++;
        end
      ...
      if(mailbox_retired&&!ipending&&!dpending&&!iv&&!dv&&!sv&&!rv)begin
        ...
        check(alias_stores==2&&alias_retirements==2&&interrupts==1&&decrements==1,"missing effects");
        check(bat_writes==25&&bat_reads==19,"missing CPU BAT accesses");
        check(word_at(BASE+32'h8000)==32'h13579bdf&&word_at(BASE+32'h28000)==32'h2468ace0,"old/new physical data corrupted");
        $display("PASS compiled runtime BAT firmware: BATwrites=%0d BATreads=%0d EXT=%0d DEC=%0d alias_stores=%0d retires=%0d reads=%0d writes=%0d cycles=%0d",bat_writes,bat_reads,interrupts,decrements,alias_stores,retires,reads,writes,cycles);$finish;
      end
    end
  end
  ...
  initial begin
    ...
    // No startup writes: only the compiled CPU program configures BATs.

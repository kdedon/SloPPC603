...
case "${1:-local}" in
  local)
    cd "${script_dir}"
    exec quartus_sta --do_report_timing ppc603e_timer_bat -c ppc603e_timer_bat
    ;;
  --docker)
    exec docker run --rm --network none --user "$(id -u):$(id -g)" \
      --volume "${repo_dir}:/work" --workdir /work/quartus/timer-bat \
      "${image}" /opt/intelFPGA_lite/quartus/bin/quartus_sta \
      --do_report_timing ppc603e_timer_bat -c ppc603e_timer_bat
    ;;

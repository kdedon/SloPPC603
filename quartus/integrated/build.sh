...
python3 "${script_dir}/../check_virtual_ports.py" "${script_dir}/ppc_integrated_measure.sv" "${script_dir}/ppc603e_integrated.qsf"
...
manifest() {
  (
    ...
    sha256sum files.f ppc603e_integrated.qsf ppc603e_integrated.qpf ppc603e_integrated.sdc build.sh collect-reports.sh ../check_virtual_ports.py
  )
}
...
if [[ "${mode}" == local ]]; then
  ...
  compile=(quartus_sh --flow compile ppc603e_integrated -c ppc603e_integrated)
else
  ...
  compile=(docker run --rm --network none --user "$(id -u):$(id -g)" --volume "${repo_dir}:/work" --workdir /work/quartus/integrated "${image}" /opt/intelFPGA_lite/quartus/bin/quartus_sh --flow compile ppc603e_integrated -c ppc603e_integrated)
fi
# Never collect a stale report left by an earlier attempt.
mkdir -p "${script_dir}/output_files"
rm -f "${script_dir}/output_files/ppc603e_integrated."{flow,map,fit,sta}.rpt
set +e
...
for stage in flow map fit sta; do
  report="${script_dir}/output_files/ppc603e_integrated.${stage}.rpt"

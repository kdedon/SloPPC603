...
for stage in flow map fit sta; do
  report="${evidence_dir}/ppc603e_integrated.${stage}.rpt"
  [[ -s "${report}" ]] || { echo "ERROR: missing ${report}" >&2; exit 1; }
done
map_report="${evidence_dir}/ppc603e_integrated.map.rpt"
...
echo "Collected integrated-subset reports; inspect RAM inference and timing before making fit or frequency claims."

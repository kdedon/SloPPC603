...
def run(build, profile="legacy"):
    ...
    if profile != 'legacy':
        lists.extend([PROJECT/'rtl/system_files.f', PROJECT/'rtl/cache_control_files.f'])
    ...
    if profile == 'disabled':
        rtlargs.append('-DREFERENCE_CACHE_DISABLED')
    ...
    if missing:
        raise RuntimeError(f'cached corpus misses implemented forms: {missing}')
    negative = []
    for field in ['gpr5', 'ram[00001000]', 'ram[000010fc]']:
        ...
        if result.returncode != 1 or f' {field}:' not in result.stderr:
            raise RuntimeError(f'cached trace corruption not diagnosed: {field}')
        negative.append(result.stderr.strip())
        corrupt.unlink()
    if frozen != {str(p): digest(p) for p in inputs}:
        raise RuntimeError('sources changed during cached reference build/run; rerun after freeze')
    match = re.search(r'PASS cached reference RTL: (.*)', (build/'rtl-run.log').read_text())
    if not match:
        raise RuntimeError('missing cache/bus coverage report')
    metrics = {name: int(value) for name, value in re.findall(r'(\w+)=(\d+)', match[1])}
    artifacts = [runner, executable, program, expected, actual]
    report = {'schema_version': 2, 'cache_profile': profile, 'comparison': 'PASS', 'header': HEADER,
              ...
              'limits': ['separate immutable instruction image and 256-byte BE data RAM',
                         ...
                         'no MMU, SMC, exception or cycle-conformance oracle'],
    ...
    print(f'PASS cached reference ({profile}): {len(rows)} retirements, {len(covered)} forms, full RAM; {metrics}')

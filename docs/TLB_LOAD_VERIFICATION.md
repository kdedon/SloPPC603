...
`make -C sim lint-tlb-seed lint-tlb-load-integration test-tlb-load-decode test-core-tlb-load test-tlb-runtime-fill-router` passed with strict Verilator warnings and assertions on 2026-09-23 (`/tmp/ppc-tlb-load-focused.log`, exit 0). The decoder oracle passed **12,366 checks**. It anchors real `-mcpu=603e` object-dump words (`7c00ffa4`, `7c003fe4`, `7c004fe4`), tests all 32 old rB fields for both opcodes, and rejects 4,032 reserved RT/RA/Rc combinations. Default-off and baseline profiles reject both opcodes; combined BAT, segment, and TLBIE options leave the decode unchanged.
...
were unchanged at acceptance. The broad log is `/tmp/ppc-tlbload-regression.log`;
source manifests are `/tmp/ppc-tlbload-rtl-inputs.json` and
`/tmp/ppc-tlbload-final-inputs.json`. These local logs are not durable CI storage.
